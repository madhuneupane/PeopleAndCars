...
import { UPDATE_CAR, GET_PEOPLE, GET_PERSON_WITH_CARS } from "../../graphql/queries";
import { useQuery } from '@apollo/client'

const { Title } = Typography;


const UpdateCar = ({ id, make, model, personId, price, year, onButtonClick }) => {
  ...
  return (
    <>
      <Title level={3} style={{ marginBottom: 20 }}>Edit Car's Info</Title>
      <Form
        form={form}
        ...
        <Form.Item shouldUpdate={true}>
          {() => (
            <Button
              ...
              htmlType="submit"
              disabled={
                (!form.isFieldTouched('year') && !form.isFieldTouched('make') && !form.isFieldTouched('model') &&
                  !form.isFieldTouched('price') && !form.isFieldTouched('person')) ||
